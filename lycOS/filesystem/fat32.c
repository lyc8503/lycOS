...
FAT32_PARTITION *new_fat32_partition(DRIVE *drv, uint32_t sector_start, uint32_t sectors) {
    ...
    trace_fat32("sectors_per_cluster: %d, root_cluster_num: %d, reserved_sector_count: %d, fat count: %d, sectors per fat: %d",
...
void iterate_fat(FAT32_PARTITION *partition) {
    ...
    trace_fat32("start: %d", start_sector);

    read_drive(partition->drv, buf, start_sector, 1);
...
void list_directories(FAT32_PARTITION *partition) {
    ...
    int i;
    for (i = 0; i < 512 * partition->sectors_per_cluster; i++) {
        printk("%02x ", buf[i]);
    }
}
